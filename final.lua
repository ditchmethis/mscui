...
local textService = GetProtectedService("TextService")
...
do
    ...
    function section:removeButton(button)
    ...
    function section:removeToggle(toggle)
        for i, module in pairs(self.modules) do
    ...
    function section:removeTextbox(textbox)
        for i, module in pairs(self.modules) do
            if module == textbox then
                table.remove(self.modules, i)
                textbox:Destroy()
                break
            end
    ...
    function section:removeKeybind(keybind)
        for i, module in pairs(self.modules) do
            if module == keybind then
                table.remove(self.modules, i)
                if self.binds[keybind] then
                    self.binds[keybind] = nil
                end
                keybind:Destroy()
                break
            end
	...
	function section:removeColorPicker(colorpicker)
		if self.colorpickers[colorpicker] then
			local tab = self.colorpickers[colorpicker].tab
			if tab then
				tab:Destroy()
			end
			self.colorpickers[colorpicker] = nil
		end

		for i, module in pairs(self.modules) do
			if module == colorpicker then
				table.remove(self.modules, i)
				colorpicker:Destroy()
				break
			end
...
function section:addSlider(title, default, min, max, callback)
    ...
    local function triggerCallback(value)
        if callback then
            callback(value, function(...)
    ...
    textbox.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            if tonumber(textbox.Text) then
                ...
                textbox.Text = tostring(value) -- reset if input invalid
            end
        end
    end)
	...
	function section:removeSlider(slider)
		for i, module in pairs(self.modules) do
    ...
    function section:addMultiDropdown(title, list, callback)
        ...
        local function updateText()
            ...
            text = text:sub(1, -3)
            search.TextBox.Text = text ~= "" and text or title
        end
        
        local function updateDropdown()
            local list = utility:Sort(search.TextBox.Text, list)
            if search.Button.Rotation == 180 then
                dropdown.List.Visible = true
                dropdown.List.Frame:ClearAllChildren()
                
                local button = utility:Create("UIListLayout", {
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    Padding = UDim.new(0, 4)
                })
                
                button.Parent = dropdown.List.Frame
                ...
                for i, value in pairs(list) do
                    ...
                    local title = utility:Create("TextLabel", {
                        ...
                        Font = Enum.Font.Gotham,
                        Text = value,
                        TextColor3 = selectedItems[value] and themes.TextColor or themes.TextColor,
                        TextSize = 12,
                ...
                local targetSize = UDim2.new(1, 0, 0, math.clamp(dropdown.List.Frame.UIListLayout.AbsoluteContentSize.Y + 73, 0, 300))
                utility:Tween(dropdown, {Size = targetSize}, 0.1, Enum.EasingStyle.Circular, Enum.EasingDirection.Out)
            else
                local targetSize = UDim2.new(1, 0, 0, 30)
                utility:Tween(dropdown, {Size = targetSize}, 0.1, Enum.EasingStyle.Circular, Enum.EasingDirection.Out)
        ...
        search.Button.MouseButton1Click:Connect(function()
            utility:Tween(search.Button, {Rotation = search.Button.Rotation == 0 and 180 or 0}, 0.1, Enum.EasingStyle.Circular, Enum.EasingDirection.Out)
            updateDropdown()
        end)
        
        search.TextBox.Focused:Connect(function()
            if search.Button.Rotation == 0 then
                utility:Tween(search.Button, {Rotation = 180}, 0.1, Enum.EasingStyle.Circular, Enum.EasingDirection.Out)
                updateDropdown()
            end
            focused = true
        end)
